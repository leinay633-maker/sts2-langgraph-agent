...
export async function updatePlan(state: RunGraphState, mcp: McpFacade): Promise<PlannerState> {
  ...
  const kbHits = await mcp.call<Array<{ file: string; preview: string }>>("search_kb", { query, limit: 3 }).catch(() => []);
  ...
  return {
    ...
    confidence: kbHits.length > 0 ? 0.82 : 0.68,
    reasons: [
      `当前阶段 ${state.stateType} 触发 Planner。`,
      ...kbHits.map((hit) => `KB: ${hit.file} - ${hit.preview}`)
    ]
  };
}
